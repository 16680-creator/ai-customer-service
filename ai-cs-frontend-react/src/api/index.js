...
export const observabilityApiWrappers = {
  ...
  getTrace: (requestId) => observabilityApi.get(`/traces/${requestId}`),
}
...
export const promptApiWrappers = {
  ...
  list: () => promptApi.get(''),
  /** 列出某场景的所有版本及内容长度摘要 */
  listVersions: (scenario) => promptApi.get(`/${scenario}`),
  /** 热切换某场景的生效版本（回滚/灰度收敛） */
  setActive: (scenario, version) =>
    promptApi.post(`/${scenario}/active`, null, { params: { version } }),
}
