...
> 本文相关变化：ChatClient 的 `.options()` / `.defaultOptions()` 只接受 Builder（不再收 build 好的实例）；Options 严格不可变，`copy()` / `fromOptions()` 移除 → 改用 `mutate()...build()`。
...
→ [06-前端开发/04-Vue3核心基础](../../06-前端开发/04-Vue3核心基础.md)
