...
> 原文索引：[01-理论](01-RAG检索增强生成.md) · [02-开发总纲](02-RAG检索增强开发.md) · [03-向量检索](03-RAG向量检索实战.md) · [04-Rerank](04-RAG进阶实战-Rerank重排序.md) · [05-混合检索](05-混合检索实战-ES-BM25与向量RRF融合.md) · [06-引用溯源](06-引用溯源实战-带出处回答.md) · [07-Tika多格式](07-文档格式扩展实战-Tika多格式解析.md) · [08-RocketMQ增量同步](08-知识库增量同步实战-RocketMQ驱动向量化.md) · [09-接口测试报告](09-ai-cs-knowledge-search接口测试报告.md) · [11-知识库新范式](11-知识库新范式-LLM-Wiki与Agentic检索.md)

---

> **可升级至 2.0（2026-09 标注）**：本文涉及 **Spring AI 1.1.4**；官方 **Spring AI 2.0.0 GA** 已于 2026-06-12 发布（地基为 Spring Boot 4 / Spring Framework 7）。
> 本文相关变化：汇总篇需按 2.0 全量校订：Tool Calling 重构、Advisor 模块改名、配置扁平化、向量库 filter converter 与 Usage 口径变化。
...
16. [范式层：RAG / GraphRAG / LLM Wiki 该怎么选](#十六范式层rag--graphrag--llm-wiki-该怎么选)
...
## 十六、范式层：RAG / GraphRAG / LLM Wiki 该怎么选

> 本章是范式层速览，完整论述（三操作 ingest/query/lint、幻觉固化、分层改造路径、面试追问）见
> [11-知识库新范式-LLM Wiki与Agentic检索](11-知识库新范式-LLM-Wiki与Agentic检索.md)。

### 16.1 "Wiki 要代替 RAG"这句话怎么理解

不是替代，而是**知识组装的时机**从查询时（query-time）前移到摄入时（compile-time）。
起因是 RAG 的三个真实痛点：无积累性（每次重新推导）、跨文档合成弱、会话间失忆。
引爆点是 Karpathy 2026-04 提出的 LLM Wiki 方案；但原方案自认适合 ≤10 万 tokens，超百万 tokens 应回到 RAG / GraphRAG。

### 16.2 三方案定位（一句话）

| 方案 | 检索对象 | 回答什么 | 组装时机 |
|------|----------|----------|----------|
| 普通 RAG | chunk + 向量 | "这段资料里怎么说？" | 查询时 |
| GraphRAG | 实体 + 关系 + 证据 chunk | "A 和 B 什么关系？" | 查询时（图在摄入时建） |
| LLM Wiki | 编译后的知识页面 + 来源 | "帮我系统理解某个主题" | **摄入时** |

选型口诀：**事实查询用 RAG，关系查询用 GraphRAG，体系理解用 LLM Wiki。**

### 16.3 三个必须记住的判断

1. **"Wiki 页 + 向量检索"里，Wiki 换掉的是检索单元（原始 chunk → 页面），不是检索手段** ——
   Embedding / 向量库 / RRF / Rerank 一个都没少，所以 §四~§六 的能力照旧需要。
2. **最大风险是幻觉固化**：RAG 的错误是一次性的，Wiki 的错误是资产化的（可扩散到 10~15 个页面）。
   防护四件套 = 强制 `sources` 留源 + 定期 `lint` + `confidence` 衰减 + Git 可回滚（能从 raw 重建）。
3. **Wiki 有硬边界**：规模 >100 万 tokens、多租户 ACL、毫秒级实时、强合规加密、逐字保真、
   高频固定事实查询——命中任意一条都该继续用 RAG。

### 16.4 分层架构（与本项目的对应）

```text
L1 原始层（真相源，不可变）    → kb_document(doc_type=RAW)
L2 编译层（知识资产，可重建）  → kb_document(doc_type=WIKI_PAGE) + GraphRagService 图谱
L3 检索层（手段，可替换）      → Chroma 向量 + ES BM25 + RRF + Rerank
```

本项目改造只需**在 L1 与 L3 之间插入 L2**：`kb_document` 加 `doc_type`、
`knowledge-doc-sync-topic` 链路复用、`citations` 复用、新增 ingest 编译步骤 + lint 定时任务（XXL-Job）+ confidence 字段。
分阶段建议 P0 先手工编译单主题（≤20 篇）验证收益，**不要一步到位全自动 ingest**。
...
| 11-知识库新范式 | RAG/GraphRAG/LLM Wiki 三方案对比、ingest/query/lint、幻觉固化、分层架构与落地路径 |

本汇总在上述专题原文之外，另合并了 Word 讲稿独有的四块内容：服务边界与职责表（§2.1）、按现象定位的排障表（§12.1）、生产化检查清单（§十三）与推荐学习顺序（§十四）。第十五章（Chroma 与 ES 对比及真实生效状态）不来自上述任何一份材料，而是代码走查结论，并据此修正了 §6.3「靠入库双写保证一致」与 §6.4「双路并行」两处与代码现状不符的表述。第十六章（RAG / GraphRAG / LLM Wiki 选型）是范式层速览，完整正文见 [11-知识库新范式](11-知识库新范式-LLM-Wiki与Agentic检索.md)，属 2026-09 新增。需要逐行代码时，回看对应专题原文。
